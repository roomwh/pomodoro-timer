...
- [ ] **Task 004: 인증 관련 페이지 UI 완성**
  ...
  - [ ] 랜딩 페이지 UI (헤드라인, 식물 성장 미리보기 정적 SVG, "시작하기"/"로그인" CTA)
  - [ ] 회원가입 페이지 UI (이메일/비밀번호/비밀번호 확인 필드, React Hook Form + Zod 클라이언트 검증)
  - [ ] 로그인 페이지 UI (이메일/비밀번호 필드, 인라인 에러 표시 영역)
  - [ ] 폼 유효성 검증 동작 확인 (이메일 형식, 최소 8자, 비밀번호 일치) — 실제 인증 호출은 더미 처리
  - [ ] 반응형 및 모바일 최적화

- [ ] **Task 005: 타이머 페이지 UI 및 식물 SVG 에셋 제작**
  - See: `/tasks/005-timer-ui-and-plant-assets.md`
  - [ ] 식물 SVG 에셋 제작: 3종(튤립/해바라기/선인장) × 4단계(씨앗/새싹/꽃봉오리/만개) + 시들기 표현
  - [ ] 타이머 페이지 상태별 UI 구현 (설정 상태 / 진행 상태 / 완료 상태)
  - [ ] 집중 시간 입력 UI (숫자 입력 + `+/-` 버튼), 카운트다운 표시(MM:SS) 영역
  - [ ] 모바일 우선 단일 컬럼 + 데스크톱 좌우 2컬럼 반응형 레이아웃
  - [ ] 포기 확인 다이얼로그, 완료 토스트 등 UI 골격 (실제 타이머 로직 없이 더미 progress로 단계 전환 시연)

- [ ] **Task 006: 정원 페이지 UI 완성**
  - See: `/tasks/006-garden-ui.md`
  - [ ] 정원 뷰: 완성된 식물 카드 격자 배치 (식물 SVG 아이콘 + 날짜 + 집중 시간, 더미 데이터)
  - [ ] 빈 정원 상태 안내 UI ("아직 심어진 식물이 없어요. 첫 집중을 시작해보세요!")
  - [ ] 기록 뷰: 날짜별 세션 목록 (완료/포기 상태, 집중 시간, 시작 시각)
  - [ ] 총 완료 세션 수 / 총 집중 시간 집계 표시 영역
  - [ ] "타이머로 돌아가기" 네비게이션 및 반응형 레이아웃
