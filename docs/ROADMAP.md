...
- [x] **Task 008: 인증 시스템 구현 (Supabase Auth + Next.js 16 Proxy)**
  - See: `/tasks/008-auth-system.md`
  - [x] 회원가입/로그인/로그아웃 실제 Supabase Auth 연동 (이메일 확인 비활성화 → 가입 즉시 자동 로그인)
  - [x] `proxy.ts`에서 만료 토큰 세션 갱신 로직 구현
  - [x] 보호 라우트(타이머/정원)는 서버 컴포넌트/레이아웃에서 `getUser()`로 서버 측 세션 검증
  - [x] 비로그인 전용 페이지(랜딩/로그인/회원가입)는 Proxy optimistic check로 로그인 사용자를 타이머로 리디렉션
  - [x] 헤더에 로그인 사용자 이메일 표시 (F011) 및 로그아웃 연동
  - **테스트 체크리스트** (Playwright 기반 E2E 또는 수동 검증)
    - [x] 회원가입 → 자동 로그인 → 타이머 페이지 리디렉션 플로우
    - [x] 잘못된 자격 증명 로그인 시 인라인 에러 표시
    - [x] 비로그인 상태로 `/timer`, `/garden` 접근 시 로그인 페이지로 리디렉션
    - [x] 로그인 상태로 `/login`, `/signup` 접근 시 타이머로 리디렉션
    - [x] 로그아웃 후 세션 무효화 및 보호 라우트 접근 차단

- [x] **Task 009: 타이머 신뢰성 로직 구현 (절대 시각 기반)**
  - See: `/tasks/009-timer-reliability.md`
  - [x] 틱 카운팅 금지 — `startTime`(절대 시각) 저장 후 `elapsed = Date.now() - startTime`로 매 틱 재계산
  - [x] `progress = elapsed / total` 계산 및 카운트다운(MM:SS)은 1초 간격 표시 갱신
  - [x] `visibilitychange` / `focus` 이벤트에서 elapsed 재계산 및 백그라운드 복귀 보정
  - [x] `elapsed >= total` 시 즉시 완료 확정 처리
  - [x] 타이머 상태 머신 구현 (설정 → 진행 → 완료/포기 → 초기화)
  - **테스트 체크리스트** (Playwright 기반 E2E 또는 수동 검증)
    - [x] 백그라운드 탭 전환 후 복귀 시 경과 시간이 실제 시각 기준으로 정확히 보정되는지 확인
    - [x] 탭이 백그라운드인 동안 종료 시각 경과 후 복귀 시 즉시 완료 확정되는지 확인
...
- [x] **Task 011: 세션 저장 및 정원 데이터 연동**
  - See: `/tasks/011-session-persistence.md`
  - [x] 타이머 완료 시 세션 'completed' 저장 + plant_type 랜덤 배정 + 정원 식물 반영
  - [x] 타이머 포기 시 세션 'abandoned' 저장
  - [x] 정원 페이지 더미 데이터를 실제 Supabase 쿼리로 교체 (식물 격자, 날짜별 기록, 집계)
  - [x] 완료 후 "정원에 심기 완료!" 토스트 및 초기 상태 복귀 연동
  - **테스트 체크리스트** (Playwright 기반 E2E 또는 수동 검증)
    - [x] 타이머 완료 → focus_sessions completed 저장 → 정원에 식물 추가 확인
    - [x] 타이머 포기 → focus_sessions abandoned 저장 확인
    - [x] 정원 뷰/기록 뷰가 실제 데이터로 렌더링되는지 확인
    - [x] 빈 정원 상태 (데이터 없는 신규 사용자) 안내 표시 확인
    - [x] 총 완료 세션 수/총 집중 시간 집계 정확도 검증

- [x] **Task 012: 핵심 기능 통합 테스트**
  - See: `/tasks/012-integration-test.md`
  - [x] 전체 사용자 플로우 E2E 테스트 (랜딩 → 회원가입 → 타이머 → 완료 → 정원)
  - [x] 인증/타이머/식물 성장/세션 저장 비즈니스 로직 통합 검증
  - [x] 에러 핸들링 및 엣지 케이스 테스트 (timer-view 저장 호출 `.catch` 누락 결함 발견·수정)
  - **테스트 체크리스트** (Playwright 기반 E2E 또는 수동 검증)
    - [x] 신규 가입자 첫 집중 완료까지 전체 여정 통과
    - [x] 백그라운드 복귀 보정과 세션 저장이 함께 동작하는 통합 시나리오
    - [x] 네트워크 실패/세션 만료 시 에러 핸들링
    - [x] RLS로 타 사용자 데이터 접근 불가 재확인

### Phase 4: 고급 기능 및 최적화

- [x] **Task 013: 사용자 경험 향상 및 에러 핸들링 강화**
  - See: `/tasks/013-ux-enhancement.md`
  - [x] 로딩/스켈레톤 상태, 빈 상태, 에러 바운더리 정교화
  - [x] 폼 제출 중 로딩/비활성화 처리 및 접근성(a11y) 개선
  - [x] 토스트/다이얼로그 UX 다듬기 및 마이크로 인터랙션 보강
  - [x] 모바일 화면 잠금 복귀 등 알려진 한계에 대한 사용자 안내 처리

- [x] **Task 014: 성능 최적화 및 Vercel 배포**
  - See: `/tasks/014-optimization-and-deploy.md`
  - [x] 식물 애니메이션 성능 최적화 및 SVG 에셋 경량화
  - [x] 페이지 로드 성능 최적화 (LCP 2.5초 이하 목표, 이미지/폰트 최적화)
  - [x] Vercel 배포 파이프라인 구성 및 환경 변수 설정
  - [x] Vercel Analytics 연동 및 성공 지표(완료율/재방문율/LCP) 모니터링 기반 마련
