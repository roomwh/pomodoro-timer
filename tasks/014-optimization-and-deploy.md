...
- [~] 식물 애니메이션이 기존과 **동일한 시각 결과**를 유지하면서, `will-change`가 진행 중에만 적용되어 idle/정원에서 불필요한 합성 레이어가 생기지 않는다(진행 중 60fps 유지 재확인). _(코드 적용: `data-animate="true"`에만 `will-change` — 시각 영향 없는 합성 힌트. 브라우저 60fps/시각 동일성 실측은 후속 통합 검증)_
...
- [ ] Vercel에 프로젝트가 연결되고 환경 변수가 등록되어 **프로덕션 URL에서 가입→로그인→타이머 완료→정원 반영**이 동작한다(Supabase Auth 리디렉션 포함). _(사용자 Vercel/Supabase 설정 필요)_
...
- [ ] 로드맵 성공 지표(완료율/재방문율/LCP)를 **어디서 확인하는지**가 문서화된다.
- [ ] 비즈니스 로직(인증/타이머 보정/세션 저장/정원 조회)이 변경되지 않았고 회귀가 없다(완료→저장→정원 반영, 포기 저장, RLS 격리 재확인).
- [ ] `npx tsc --noEmit`·`npm run lint`·`npm run build` 통과(콘솔 에러/경고 없음, lockfile 경고 해소 포함).
...
- [ ] **(사용자 액션) Vercel 연결·환경 변수·Supabase Auth URL**: GitHub 리포 import(또는 `vercel` 링크), env 2종 등록, Supabase Site/Redirect URL에 배포 도메인 추가 — 절차 안내 후 사용자 수행
- [ ] **프로덕션 smoke test**: 배포 URL에서 가입→로그인→타이머 완료→정원 반영 전 여정 통과, Analytics/Speed Insights 수집 확인, 지표 확인 위치 문서화(README)
- [ ] 핵심 흐름 회귀 점검 및 `npx tsc --noEmit`·`npm run lint`·`npm run build` 최종 통과 확인
...
- [ ] **애니메이션 시각 동일성**: 경량화 후에도 씨앗→새싹→꽃봉오리→만개 전환·시들기·꽃잎 펼침(bloom)이 기존과 동일하게 보이고 진행 중 60fps가 유지되는지 확인
- [ ] **합성 레이어 범위**: idle/정원에서 `will-change`로 인한 상시 레이어 승격이 사라지고, running 중에만 적용되는지 확인(시각 영향 없음)
- [x] **미사용 에셋 제거 무결성**: `public/*.svg` 삭제 후 빌드·런타임에 깨진 참조/404가 없는지 확인 _(grep 코드 미참조 확인, `npm run build` 성공)_
- [x] **LCP 실측**: 프로덕션 빌드에서 랜딩·타이머·정원의 LCP가 2.5초 이하인지 Lighthouse/Playwright로 확인(초과 시 원인·대응 기록) _(랜딩 웜 384ms·콜드 1608ms로 충족, LCP=H1. timer/garden은 6단계 smoke test에서 인증 상태로 확인 예정)_
- [x] **빌드 클린**: `npm run build`가 lockfile workspace-root 경고 없이 통과하는지 확인 _(build·`next start` 출력에 경고 없음 확인)_
- [ ] **프로덕션 인증 여정**: 배포 URL에서 회원가입→자동 로그인→`/timer` 리디렉션→완료→정원 반영이 Supabase Auth 리디렉션 포함 정상 동작하는지 확인 _(사용자 설정 필요)_
- [~] **Analytics 수집**: 프로덕션에서 페이지뷰·Web Vitals가 수집되고 완료 커스텀 이벤트가 발사되는지(개발 환경은 미전송) 확인 _(로컬: 스크립트 주입·window.va/si·`track` 호출 경로 확인. 실제 수집은 Vercel 배포 후 6단계에서 대시보드로 확인)_
- [ ] **회귀 점검**: 완료→저장→정원 반영, 포기 저장, 보호 라우트 게이트, RLS 격리가 기존대로 동작하는지(로직 무변경) 재확인
